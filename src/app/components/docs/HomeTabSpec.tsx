    ...
    <div className="min-h-screen bg-gray-50 font-sans text-sm">
      ...
      <div className="px-3 py-4 space-y-4">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-3 py-3">
          <p className="text-xs font-bold text-blue-900 mb-1">🧭 비회원(Base) 기준 정책</p>
          <ul className="text-[11px] text-blue-900 space-y-1 list-disc list-inside leading-relaxed">
            ...
            <li><strong>문서 운영:</strong> 본 문서는 기본 동작의 기준 스펙이며, 회원 전용 차이점은 회원 탭(HomeTabSpecBiz)에서만 다룹니다.</li>
          </ul>
        </div>
        ...
        <Section id="layout" title="2. 화면 구성 (위에서 아래 순서)" color="indigo">
          ...
          <SubSection label="구역 4" title="커뮤니티 추천글 가로 스크롤">
            ...
            <div className="mt-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
              <p className="text-sm font-bold text-slate-900 mb-2">👥 비회원(Base) 기준 — 추천글 섹션</p>
              <Table
                ...
                rows={[
                  ...
                  ['상세: 게시글 공감', '❌ 탭 시 로그인 유도', '회원 확장 동작은 회원 탭에서 정의'],
                  ['상세: 댓글·답글', '❌ 입력 비활성·하단 안내', '회원 확장 동작은 회원 탭에서 정의'],
                  ['상세: ⋮ 더보기 메뉴', '공유하기만 노출', '신고/수정/삭제 등은 회원 탭에서 정의'],
                  ['더보기 → 커뮤니티 탭', '✅ 이동', '구역 4는 샘플 5개, 커뮤니티 탭은 전체 목록'],
                ]}
              />
              <p className="mt-3 text-xs text-slate-700">
                회원 전용 확장(열람 한도 완화, 공감/댓글 작성, 신고·수정/삭제)은 회원 탭(HomeTabSpecBiz)의 Delta 정책을 참조합니다.
              </p>
            </div>
        ...
        <Section id="modals" title="3. 팝업 설명" color="violet">
          ...
          <SubSection label="팝업 2" title="커뮤니티 게시글 상세 (홈·통합검색 경로)">
            ...
            <div className="mt-3 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
              <p className="text-sm font-bold text-emerald-800 mb-3">👥 비회원(Base) 기준 — 상세 모달</p>
              ...
              <p className="mt-3 text-xs text-emerald-800">회원 확장 동작은 회원 탭(`HomeTabSpecBiz`)의 Delta 정책을 참조합니다.</p>
            </div>
          </SubSection>
          ...
          <SubSection label="공통" title="로그인 유도 안내 🔗 공통 컴포넌트">
            ...
            <Table
              ...
              rows={[
                ...
                ['상세 스펙', '📄 공통 컴포넌트 정의서 참조', '표시 형태, 문구 등'],
              ]}
            />
            <Note>
              👉 로그인 유도 모달에서 로그인(가입) 버튼 탭 시 이후의 진행 흐름은 마이페이지 화면정의서에 정리된 [공통 로그인/회원가입 플로우 정책]을 공통으로 따름.
            </Note>
          </SubSection>
        </Section>

        {/* ───── 4. Base 기준 기능 요약 ───── */}
        <Section id="auth" title="4. 비회원(Base) 기준 기능 요약" color="emerald">
          <Table
            headers={['기능', '비회원(Base)', '비고']}
            rows={[
              ['홈 화면 전체 보기', '✅ 가능', '—'],
              ...
              ['의사 리뷰쓰기', '❌ 로그인 유도', '회원 확장 기능'],
              ['커뮤니티 추천글 읽기', '✅ 가능 (전역 한도 정책 적용)', '—'],
              ['커뮤니티 게시글 공감/댓글', '❌ 로그인 유도 또는 입력 비활성', '회원 확장 기능'],
              ['커뮤니티 게시글 ⋮ 더보기 메뉴', '공유하기만 (신고 숨김)', '회원 확장 기능'],
              ['건강 정보 열람', '✅ 가능', '외부 링크로 즉시 이동'],
            ]}
          />
          <Note>
            회원 전용 차이점(Delta)은 회원 탭(HomeTabSpecBiz)에서만 관리합니다.
          </Note>
        </Section>
